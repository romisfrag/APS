...
and printeXPR e =
  ...
  | Lambda (a,e) -> "lambda([" ^ printaRGS a ^ "]," ^ printeXPR e ^ ")"
  | Application e -> "application([" ^ printeXPRS e ^ "])"
and printeXPRS e =
  match e with
  | Expr e -> printeXPR e  
  | Exprs (e,eS) -> printeXPR e ^ "," ^ printeXPRS eS
and printaRGS a =
  match a with
  | Arg a -> printaRG a
  | Args (a,aS) -> printaRG a ^ "," ^ printaRGS aS
and printaRG a =
  match a with
  | Argument (i,t) -> "arg(" ^ printiDENT i ^ "," ^ printtYPE t ^ ")" 
and printuNOP u =
  match u with   
...
and printtYPE t =
  ...
  | Bool -> "bool"
  | Int -> "int"
  | Fleche (tS,t) -> "fleche([" ^ printtYPES tS ^ "]," ^ printtYPE t ^ ")"
and printtYPES t =
  match t with
  | Type t -> printtYPE t
  | Types (t,tS) -> printtYPE t ^ "," ^ printtYPES tS
and printiDENT i =
  match i with
  | Ident s -> s
